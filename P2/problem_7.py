...
class RouteTrie:
    ...
    def insert(self, route, leaf_handler):
        # Similar to our previous example you will want to recursively add nodes
        # Make sure you assign the handler to only the leaf (deepest) node of this path
        current_route = self.root

        route_sliced = route[:-1] if route.endswith("/") else route

        for sub_route in route_sliced.split("/"):
            if sub_route not in current_route.children:
                current_route.insert(sub_route)
            current_route = current_route.children[sub_route]
    ...
    def find(self, path):
        ...
        for sub_path in path_sliced.split("/"):
